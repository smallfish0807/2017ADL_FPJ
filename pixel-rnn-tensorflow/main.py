...
flags.DEFINE_boolean("use_discrete", False, "whether to use discrete data")

# data
flags.DEFINE_string("data", "imageNet", "name of dataset [mnist, cifar, imageNet]")
...
def main(_):
    ...
    if conf.data == "mnist":
        ...
        if conf.use_discrete:
            file_name = './npy/images_all_valid_discrete'
        else:
            file_name = './npy/images_all_valid'
        try:
            images_all = np.load(file_name + '.npy')
        except (FileNotFoundError, OSError):
            images_all = get_all_imageNet_images('./data/small/', conf.use_discrete)
            np.save(file_name, images_all)
...
<<<<<<< HEAD
                            images[:, height//2:, :, :] = 0. # remove lower half of images
                            save_images(images, height, width, 10, 10, directory=SAMPLE_DIR, prefix="test_ori")
                            print('ya')
                            input()
=======
                            save_images(prob2image(images, conf.use_discrete), height, width, 10, 10, directory=SAMPLE_DIR, prefix="test_ori")
                            images_half = images.copy()
                            images_half[:, height//2:, :, :] = 0.
                            save_images(prob2image(images_half, conf.use_discrete), height, width, 10, 10, directory=SAMPLE_DIR, prefix="test_ori_half")
>>>>>>> d529717450647d421244cfeccd01fd237e0710a1
                        if epoch % 2 == 0 and idx == 0:
                            test_out = network.predict(images)
                            save_images(prob2image(test_out, conf.use_discrete), height, width, 10, 10, directory=SAMPLE_DIR, prefix="test_out_epoch_%s" % epoch)
                            test_out_half = network.generate_half(images)
                            save_images(prob2image(test_out_half, conf.use_discrete), height, width, 10, 10, directory=SAMPLE_DIR, prefix="test_out_half_epoch_%s" % epoch)
